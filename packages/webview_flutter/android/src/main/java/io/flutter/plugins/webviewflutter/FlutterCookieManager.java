...
class FlutterCookieManager implements MethodCallHandler {
  ...
  private static void setCookies(final MethodCall methodCall, final Result result) {
    ...
    for (Map<String, Object> cookieMap : serializedCookies) {
      cookieManager.setCookie(cookieMap.get("domain").toString(), cookieMap.get("asString").toString());
    }
  ...
  private static HttpCookie mapToCookie(Map<String, Object> input) {
    final HttpCookie cookie =
        new HttpCookie(String.valueOf(input.get("name")), String.valueOf(input.get("value")));
    cookie.setDomain(String.valueOf(input.get("domain")));
    cookie.setPath(String.valueOf(input.get("path")));
    cookie.setSecure(Boolean.parseBoolean(String.valueOf(input.get("secure"))));
    if (Build.VERSION.SDK_INT >= VERSION_CODES.N) {
      cookie.setHttpOnly(Boolean.parseBoolean(String.valueOf(input.get("httpOnly"))));
    }
